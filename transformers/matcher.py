...
class JobMatcher:
    ...
    def should_fetch_details(
        ...
        avoid_score, reasons = self.score_keywords(
            text,
            self.profile["avoid"],
            "avoid"
        )
        ...
        if not keep:
            logger.info(
                f"Pulando detalhe de '{job.get('title')}' — {reasons}"
            )
    ...
    def rank_jobs(
        ...
        logger.info(
            f"Ranking concluído: {len(ranked)} vagas"
        )
